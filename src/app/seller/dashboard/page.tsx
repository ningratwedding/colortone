...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const revenueByMonth = last6Months.map(monthDate => {
          ...
          const monthlyRevenue = fetchedSellerOrders.reduce((acc, order) => {
    ...
    const salesByProduct = allSellerOrders.reduce((acc, order) => {
      acc[order.productId] = (acc[order.productId] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const productsWithSales = sellerProducts.map(p => ({
      ...p,
  ...
  }, [sellerProducts, allSellerOrders]);
